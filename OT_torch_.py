...
def GW_distance(X, Y, p, q, beta, lamda=0.5, iteration=5, OT_iteration=20, **kwargs):
	'''
	...
	:return: GW distance
	'''
	Cs = cos_batch_torch(X, X, beta).float().cuda()
	Ct = cos_batch_torch(Y, Y, beta).float().cuda()
...
def GW_distance_uniform(X, Y, beta, lamda=1e-1, iteration=5, OT_iteration=20, **kwargs):
	m = X.size(2)
	n = Y.size(2)
	bs = X.size(0)
	p = (torch.ones(bs, m, 1)/m).cuda()
	q = (torch.ones(bs, n, 1)/n).cuda()
	return GW_distance(X, Y, p, q, beta, lamda=lamda, iteration=iteration, OT_iteration=OT_iteration, **kwargs)
